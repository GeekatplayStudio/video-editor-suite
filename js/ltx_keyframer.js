...
// Global registry to track all GAPKeyframer nodes across all subgraphs
window._GAPKeyframerGlobalNodes = window._GAPKeyframerGlobalNodes || new Set();
...
// Finds all other GAPKeyframer nodes globally and mirrors the value to them
function syncWidgetAcrossNodes(sourceNode, widgetName, value) {
    if (!window._GAPKeyframerGlobalNodes) return;
    
    for (const targetNode of window._GAPKeyframerGlobalNodes) {
        // Target all OTHER GAPKeyframer nodes by direct object reference
...
app.registerExtension({
    name: "Comfy.GAPKeyframer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "GAPKeyframer") return;

        // Register this node instance globally
        window._GAPKeyframerGlobalNodes.add(node);
        ...
        // Helper: read image count from a connected GAPMultiImageLoader node
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "GAPMultiImageLoader") {
                    return originNode;
                }
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode && innerNode.comfyClass === "GAPMultiImageLoader") {
                            return innerNode;
                        }
                    } catch (e) {
            ...
            // If there is EXACTLY ONE GAPMultiImageLoader in the workspace, safely assume that's the one.
            let multiImageLoaders = [];
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "GAPMultiImageLoader") {
        ...
        node.onRemoved = function() {
            ...
            window._GAPKeyframerGlobalNodes.delete(node);
