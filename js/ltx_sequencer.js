...
// Global registry to track all GAPSequencer nodes across all subgraphs
window._GAPSequencerGlobalNodes = window._GAPSequencerGlobalNodes || new Set();
...
function syncFullStateAcrossNodes(sourceNode) {
    if (!window._GAPSequencerGlobalNodes) return;
    
    for (const targetNode of window._GAPSequencerGlobalNodes) {
...
app.registerExtension({
    name: "Comfy.GAPSequencer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "GAPSequencer") return;

        // Register this node instance globally
        window._GAPSequencerGlobalNodes.add(node);
        ...
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "GAPMultiImageLoader") return originNode;
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode?.comfyClass === "GAPMultiImageLoader") return innerNode;
                    } catch (e) {}
                }
                return null;
            }
            ...
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "GAPMultiImageLoader") multiImageLoaders.push(n);
        ...
        node.onRemoved = function() {
            window._GAPSequencerGlobalNodes.delete(node);
            clearInterval(pollInterval);
