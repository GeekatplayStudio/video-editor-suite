...
PREVIEW_ROUTE = "/gap_video_suite/preview"
UPLOAD_ROUTE = "/gap_video_suite/upload_chunk"
...
def _safe_upload_name(filename: str) -> str:
    safe_name = re.sub(r"[^A-Za-z0-9._-]+", "_", Path(str(filename or "")).name).strip("._")
    if not safe_name:
        raise web.HTTPBadRequest(text="Invalid filename")
    return safe_name


def _resolve_preview_path(filename: str) -> Path | None:
    if not filename:
        return None
    ...
    try:
        if candidate.is_absolute():
            ...
            candidate = (input_dir / candidate).resolve(strict=True)
...
# Preview files only from ComfyUI's input directory.
@PromptServer.instance.routes.get(PREVIEW_ROUTE)
async def custom_view(request):
    file_path = _resolve_preview_path(request.query.get("filename", ""))
    if file_path is not None:
        return web.FileResponse(file_path)
    return web.Response(status=404, text="File not found")

# Chunked uploads are written into ComfyUI's input directory with sanitized names.
@PromptServer.instance.routes.post(UPLOAD_ROUTE)
async def upload_chunk(request):
    post = await request.post()
    file = post.get("file")
    filename = _safe_upload_name(post.get("filename"))
    chunk_index = int(post.get("chunk_index", 0))
    total_chunks = int(post.get("total_chunks", 1))

    if file is None or not hasattr(file, "file"):
        raise web.HTTPBadRequest(text="Missing upload chunk")
    if chunk_index < 0 or total_chunks < 1 or chunk_index >= total_chunks:
        raise web.HTTPBadRequest(text="Invalid chunk metadata")

    file_path = _input_dir() / filename
    ...
    def load_video(self, video, frame_rate, display_mode, start_time, end_time, duration, start_frame, end_frame, duration_frames, custom_width=0, custom_height=0, resize_method="maintain aspect ratio", crop_x=0.0, crop_y=0.0, crop_w=1.0, crop_h=1.0, **kwargs):
        ...
        if video_stream:
            ...
            for frame in container.decode(video_stream):
                ...
                try:
                    ...
                    print(f"[GAPLoadVideoUI] Color reformat failed, using default: {e}")
                    frame_rgb = frame.to_ndarray(format='rgb24')
                ...
                while expected_target_time <= frame_time and expected_target_time < actual_end_time - 1e-5:
                    if image_tensor is None and expected_frames > 0:
                        ...
                        try:
                            ...
                            print(f"[GAPLoadVideoUI] Pre-allocation failed, falling back to list: {e}")
        ...
        if len(container.streams.audio) > 0:
            try:
                ...
                print(f"[GAPLoadVideoUI] Audio track extraction skipped or failed: {e}")
